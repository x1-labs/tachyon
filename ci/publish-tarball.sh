...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/tachyon-install-init tachyon-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml tachyon-install-init-"$TARGET"* $MAYBE_TARBALLS; do
