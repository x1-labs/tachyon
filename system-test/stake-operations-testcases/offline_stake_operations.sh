...
x1-keygen new -o "$dummy_keypair" --no-passphrase --force --silent
x1 config set --keypair $dummy_keypair
...
x1-keygen new -o "$online_system_account_keypair" --no-passphrase --force --silent
x1-keygen new -o "$online_nonce_account_keypair" --no-passphrase --force --silent

online_system_account_pubkey="$(x1-keygen pubkey $online_system_account_keypair)"
nonce_account_pubkey="$(x1-keygen pubkey $online_nonce_account_keypair)"
...
x1-keygen new -o "$offline_system_account_keypair" --no-passphrase --force --silent
x1-keygen new -o "$offline_staker_keypair" --no-passphrase --force --silent
x1-keygen new -o "$offline_withdrawer_keypair" --no-passphrase --force --silent
x1-keygen new -o "$offline_custodian_keypair" --no-passphrase --force --silent

offline_system_account_pubkey="$(x1-keygen pubkey $offline_system_account_keypair)"
offline_withdrawer_pubkey="$(x1-keygen pubkey $offline_withdrawer_keypair)"
offline_staker_pubkey="$(x1-keygen pubkey $offline_staker_keypair)"
offline_custodian_pubkey="$(x1-keygen pubkey $offline_custodian_keypair)"

# Airdrop some funds to the offline account.
x1 airdrop 100 $offline_system_account_pubkey
x1 airdrop 2 $online_system_account_pubkey

# Create a nonce account funded by the online account, with the authority given to the offline account
x1 create-nonce-account $online_nonce_account_keypair 1 --nonce-authority $offline_system_account_pubkey --keypair $online_system_account_keypair
nonce="$(x1 nonce $nonce_account_pubkey)"

execution_step OFFLINE SYSTEM ACCOUNT BALANCE BEFORE CREATING STAKE ACCOUNTS
(
  set -x
  x1 balance $offline_system_account_pubkey
)
...
x1-keygen new -o "$stake_account_keypair" --no-passphrase --force --silent
stake_account_address="$(x1-keygen pubkey $stake_account_keypair)"

sign_only="$(x1 create-stake-account $stake_account_keypair 50 \
...
x1 create-stake-account $stake_account_keypair 50 \
...
(
  set -x
  x1 stake-account $stake_account_address
)
...
x1-keygen new -o $split_stake_account_keypair --no-passphrase --force --silent
split_stake_account_address=$(x1-keygen pubkey $split_stake_account_keypair)

nonce="$(x1 nonce $nonce_account_pubkey)"

sign_only="$(x1 split-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
x1 split-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
...
(
  set -x
  x1 stake-account $split_stake_account_address
)
...
sign_only="$(x1 stake-set-lockup --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
  $split_stake_account_address --custodian $offline_custodian_keypair --lockup-epoch 0 \
  --keypair $offline_system_account_keypair --sign-only --url http://0.0.0.0)"

signers="$(get_signers_string "${sign_only[@]}")"

x1 stake-set-lockup --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
sign_only="$(x1 withdraw-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
x1 withdraw-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
  $split_stake_account_address $offline_system_account_pubkey 10 \
...
vote_account_pubkey="$(awk '{if(NR==4) print $2}'<<<"$(x1 show-validators)")"
nonce="$(x1 nonce $nonce_account_pubkey)"

# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(x1 delegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
x1 delegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
...
# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(x1 deactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
# Send the signed transaction on the cluster
x1 deactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
