...
### Primary Validator
The following additional `tachyon-validator` parameters are required to enable
...
following `tachyon-validator` command-line argument changes:
...
```bash
$ tachyon-validator wait-for-restart-window --identity validator-keypair.json \
  && tachyon-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `tachyon-validator set-identity
