...
the `--cuda` argument to `tachyon-validator`.
...
argument to `tachyon-validator`. You can specify multiple ones by repeating the argument `--known-validator <PUBKEY1> --known-validator <PUBKEY2>`.
...
> `tachyon-validator --identity ASK ... --authorized-voter ASK ...`
...
example, `tachyon-validator --dynamic-port-range 11000-11020 ...` will restrict
...
`tachyon-validator` command-line. Ensure that the 'exec' command is used to
start the validator process (i.e. "exec tachyon-validator ..."). This is
...
The validator log file, as specified by `--log ~/tachyon-validator.log`, can get
...
launch the process with `exec` (`exec tachyon-validator ...`) when using logrotate.
...
/home/sol/tachyon-validator.log:
...
Now add the `--accounts /mnt/solana-accounts` argument to your `tachyon-validator`
