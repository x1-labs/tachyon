...
From within the Solana CLI, you can execute the `tachyon-validator` command with
the `--help` flag to get a better understanding of the flags and sub commands
available.

```
tachyon-validator --help
```
...
Assuming you are ready to restart, you may use the `tachyon-validator exit`
...
your `tachyon-validator` binary to your specific hardware.
...
the newly installed version is in use. Use `tachyon-validator exit` to restart
