...
```bash
x1-keygen new --no-passphrase -o stake-account.json
```
...
```bash
x1 create-stake-account --from <KEYPAIR> stake-account.json <AMOUNT> \
...
View the new stake account with the `x1 stake-account` command:
...
`--withdraw-authority` options, or afterward with the `x1 stake-authorize`
command. For example, to set a new stake authority, run:

```bash
x1 stake-authorize <STAKE_ACCOUNT_ADDRESS> \
...
```bash
x1 create-stake-account --from <KEYPAIR> <STAKE_ACCOUNT_KEYPAIR> --seed <STRING> <AMOUNT> \
...
`x1 create-address-with-seed`:

```bash
x1 create-address-with-seed --from <PUBKEY> <SEED_STRING> STAKE
```

`<PUBKEY>` is the public key of the `<STAKE_ACCOUNT_KEYPAIR>` passed to
`x1 create-stake-account`.
...
accounts with the `x1 validators` command:
...
```bash
x1 delegate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <VOTE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
After delegating stake, use `x1 stake-account` to observe the changes to the
stake account:
...
Once delegated, you can undelegate stake with the `x1 deactivate-stake`
command:

```bash
x1 deactivate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
Transfer tokens out of a stake account with the `x1 withdraw-stake` command:

```bash
x1 withdraw-stake --withdraw-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <RECIPIENT_ADDRESS> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
...
existing stake account to a new one, use the `x1 split-stake` command:

```bash
x1 split-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <NEW_STAKE_ACCOUNT_KEYPAIR> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
