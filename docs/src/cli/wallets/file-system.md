...
Use Solana's command-line tool `x1-keygen` to generate keypair files. For
example, run the following from a command-line shell:

```bash
mkdir ~/my-solana-wallet
x1-keygen new --outfile ~/my-solana-wallet/my-keypair.json
```
...
```bash
x1-keygen pubkey ~/my-solana-wallet/my-keypair.json
```
...
```bash
x1-keygen verify <PUBKEY> ~/my-solana-wallet/my-keypair.json
```
