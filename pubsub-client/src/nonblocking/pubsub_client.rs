...
//! `--rpc-pubsub-enable-vote-subscription` to `tachyon-validator`. When these
...
impl PubsubClient {
    ...
    /// `--rpc-pubsub-enable-block-subscription` to `tachyon-validator`.
    ///
    /// # RPC Reference
    ///
    ...
    ///
    /// This method is disabled by default. It can be enabled by passing
    /// `--rpc-pubsub-enable-vote-subscription` to `tachyon-validator`.
    ///
    /// # RPC Reference
    ///
